...
inputmode는 입력 필드에 <b>어떤 종류의 데이터 입력을 기대</b>하는지를 브라우저에 알려주는 속성입니다. </br>
이를 통해 <b>모바일 장치</b>에서 적절한 <u>키보드 레이아웃(예: 숫자 키패드, 전화번호 키패드, 이메일 입력용 키보드 등)이 표시</u>되도록 설정할 수 있습니다.

- text: 기본 텍스트 입력 (기본값).
- numeric: 숫자 입력용 키패드 (0-9, 소수점 등).
- decimal: 소수점 포함 숫자 입력용 키패드.
- tel: 전화번호 입력용 키패드.
- email: 이메일 입력에 최적화된 키보드.
- url: URL 입력에 최적화된 키보드.
- search: 검색 입력에 최적화된 키보드.
